...
class FlashInferMLASparseSM120Impl(MLAAttentionImpl[FlashInferMLASparseMetadata]):
    ...
    def forward_mqa(
        ...
        # Pass the actual index width to FlashInfer, but clamp to the SM120
        # GLM_NSA instantiation (topk==2048). Wider buffers (e.g. RedHatAI
        # 2176) have no kernel; prefer --hf-overrides index_topk=2044 so the
        # buffer is allocated at 2048 and the kpool tail still fits.
        _SM120_GLM_NSA_TOPK = 2048
        eff_topk = topk_indices_physical.shape[-1]
        if eff_topk > _SM120_GLM_NSA_TOPK:
            topk_indices_physical = topk_indices_physical[:, :_SM120_GLM_NSA_TOPK]
            eff_topk = _SM120_GLM_NSA_TOPK
