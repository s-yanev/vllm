...
class FlashInferMLASparseSM120Backend(_FlashInferMLASparseBackendBase):
    ...
    def supports_combination(
        ...
        if vllm_config.model_config is not None:
            ...
            # FlashInfer SM120 GLM_NSA kernels are only instantiated for
            # topk==2048 (see supported_sparse_mla_sm120_configs). Buffer
            # width = index_topk + (kpool-1) padded to a multiple of 128:
            # - official / overridden: index_topk=2044, kpool=4 → 2047 → 2048
            # - RedHatAI NVFP4 default: index_topk=2048, kpool=4 → 2051 → 2176
            #   → unsupported; override to 2044 via
            ...
            if eff_width != 2048:
                return (
                    "FLASHINFER_MLA_SPARSE_SM120 requires effective topk "
                    f"buffer width 2048 (FlashInfer GLM_NSA instantiation); "
                    f"got {eff_width} (index_topk={index_topk}, "
                    f"index_kpool={kpool}). For RedHatAI/GLM-5.3-Flash-NVFP4 "
                    "pass --hf-overrides "
